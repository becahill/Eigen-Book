...
This document describes events generated by Eigen-Book commands and the native
snapshot/journal recovery contract. It does not describe ingestion of an
exchange feed. The external PPO/replay path uses a separate initial-snapshot
plus sequenced-depth/trade tape; its source continuity, atomic-event, fill, and
model-artifact rules are in
[`sequenced-depth-ml.md`](sequenced-depth-ml.md).
...
- price, aggregate quantity, order count, and every FIFO order's id, quantity,
  timestamp, sequence, participant id, post-only flag, initial resting
...
Snapshot format version 4 is a deterministic little-endian stream. Book
snapshots include effective venue configuration, participant/post-only order
state, each live order's persistent `initial_quantity` and `state`, FIFO
sequence, command-event sequence, and current market-data sequence.
...
Version compatibility is exact: a v4 reader rejects all other versions with
`SnapshotVersionMismatch`. Version 3 snapshots are intentionally incompatible
because they do not persist `Order::initial_quantity` or `Order::state`;
version 2 also lacks participant, post-only, market-data sequence, and effective
venue configuration. A migration tool must decode an older contract
explicitly; the engine does not infer missing fields.
...
The matching engine owns validation, mutation, bounded event/record generation,
...
state comparison, but snapshot serialization/restore, journal record generation,
